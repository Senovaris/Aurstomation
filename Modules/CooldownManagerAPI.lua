-- CooldownManagerAPI.lua - Nine-slice frame borders and utilities
-- Adapted for Aurs Toolkit

local ADDON_NAME = "Aurs_Toolkit"
AursToolkit = AursToolkit or {}
AursToolkit.CooldownManagerAPI = AursToolkit.CooldownManagerAPI or {}

local API = AursToolkit.CooldownManagerAPI

-- Nine-slice frame mixin for custom borders
...
function SliceFrameMixin:CreatePieces(n)
    ...
    elseif n == 9 then
        -- Full 9-slice (corners + edges + center)
...
local NineSliceLayouts = {
    Dark = true,
    -- You can add more texture styles here if you create them
}

---Create a nine-slice frame with custom border texture
---@param parent Frame Parent frame to attach the nine-slice to
---@param layoutName? string Layout name from NineSliceLayouts (default: "Dark")
---@return Frame nineSliceFrame Frame with SliceFrameMixin methods
function API:CreateNineSliceFrame(parent, layoutName)
    ...
    -- Use a default texture path - you can customize this
    -- For now, we'll use a simple white texture that can be colored
    f:SetTexture("Interface\\Buttons\\WHITE8x8")
    f:SetColor(0.1, 0.1, 0.1) -- Dark gray
    
    f:ClearAllPoints()
    return f
end

---Refresh the cooldown manager system (toggle it off and on)
function API:RefreshCooldownManager()
    if C_CVar and C_CVar.GetCVar("cooldownViewerEnabled") == "1" then 
