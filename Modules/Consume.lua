...
local function UpdateHealthStoneFrame()
	local bagID, slotID, itemID = hasHealthStone()
	if bagID and slotID then
		local info = C_Container.GetContainerItemInfo(bagID, slotID)
		if info and info.iconFileID then
			...
			local hCharges = C_Item.GetItemCount(itemID, false, true)
			if hCharges and hCharges > 1 then
				hStone.count:SetText(hCharges)
				hStone.count:Show()
			else
				hStone.count:Hide()
			end
...
function UpdateConsumeLayout()
	...
	local stack = {}
	if visible1 then
		table.insert(stack, pot)
	end
	if visible3 then
		table.insert(stack, hPot)
	end
	if visible4 then
		table.insert(stack, poPot)
	end
	if visible2 then
		table.insert(stack, hStone)
	end
...
hPot.count = hPot:CreateFontString(nil, "OVERLAY", "NumberFontNormalSmall")
...
hStone.count = hStone:CreateFontString(nil, "OVERLAY", "NumberFontNormalSmall")
hStone.count:SetSize(15, 15)
hStone.count:SetPoint("BOTTOMRIGHT", hStone, "BOTTOMRIGHT", 0, 1)
hStone.count:Hide()
...
poPot.count = poPot:CreateFontString(nil, "OVERLAY", "NumberFontNormalSmall")
