...
crossIcon:SetPoint("CENTER", UIParent, "CENTER", 0, -20)
crossIcon:SetTexture("Interface\\AddOns\\Aurstomation\\Media\\Crossss.tga")
...
local function IsFriend(name)
	...
	for i = 1, numBNet do
		...
		for j = 1, numAccounts do
			...
			if gameInfo and gameInfo.characterName and StripRealm(gameInfo.characterName) == shortName then
				return true
			end
		end
	end
...
expFilter:SetScript("OnEvent", function()
	C_Timer.After(0.1, function()
		...
		if not searchBar or not searchBar:IsShown() then
			return
		end
